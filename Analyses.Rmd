...
The main different result would be that the Cursor Jump group is now statistically different from the Non-Instructed group, within the second block. This suggests that the Cursor Jump group's rate of learning was faster than the Non-Instructed group for this block, but this does not provide any further advantage, as the magnitude of asymptotic learning across all groups do not differ from each other by the end of adaptation training. Support for this comes from the lack of statistical significance for comparisons within the last block (as shown above), as well as an additional One-way ANOVA we conduct on reach deviations for only the last block (as shown below).
...
We see from the figure that when including or excluding the strategy learned to counter the rotation, only the non-instructed group could not switch their strategy on or off at will. This suggests unawareness of learning for this group. Importantly, all groups still showed angular reach deviations to some degree during without strategy trials, showing that implicit reach aftereffects were still observed in all groups. Note, however, that implicit reach aftereffects were smaller for the Hand View group compared to the other groups.
...
To investigate which group differed from each other in the exclusive (without strategy) no-cursor trials, we conducted pairwise comparisons of each group compared to the Non-instructed group, as well as a comparison between the Cursor Jump and Hand View group.

```{r}
getNoCurComparisonEffSize(type='exclusive')
```

We find that the Cursor Jump and Hand View groups are significantly different from the Non-instructed group, suggesting that implicit reach aftereffects for these groups are lower. Moreover, a significant difference between the Cursor Jump and Hand View group, suggests how implicit reach aftereffects are further reduced for the Hand View group.
...
We see that there is a main effect of group and movement type, but no significant interaction. This suggests that the effects of movement type on shifts in hand position estimates were not modulated by group, and vice-versa. Thus, we investigate group differences within Active and Passive Localization shifts separately.

## Active Localization

We conduct a one way ANOVA to test the effect of group on active localization changes.

```{r}
localizationANOVA(test='active')
```

We observe a statistical significance, suggesting that at least one group has different mean shifts in active localization, compared to the other groups. We investigate this further by conducting follow-up tests, comparing mean active localization shifts for each group to the control, Non-Instructed group.

```{r}
getactivelocComparisonEffSize(method='sidak')
```

Results from these follow-up comparisons show that only the Hand View group significantly differed from mean active localization shifts of the Non-Instructed group. Given that shifts in active localization are informed by changes in both proprioception and predictions, we further investigate passive localization shits and updates in predictions.

## Proprioceptive Recalibration

First, we run a one way ANOVA to test for the effect of group on passive localization changes.

```{r}
localizationANOVA(test='passive')
```

We see that there are no significant differences across the groups. However, we also wanted to test whether each group showed shifts in passive localization. We ran t-tests that compared each group's magnitude of shifts in passive localization estimates against 0. These were one-tailed t-tests, where the null hypothesis was that the mean is greater than or equal to 0; the alternative hypothesis was that the mean is less than 0. Hence confidence intervals are all from minus infinity. Note that we do not need to control for multiplicity here, as each group is independently compared to zero (i.e., these are not pairwise comparisons).
...
Next, we ran a one way ANOVA to test for the effect of group on changes in predicted sensory consequences. For this ANOVA, we obtained measures of changes in predicted sensory consequences by subtracting the shifts in passive localization from active localization.

```{r}
predictedConsequencesANOVA()
```

There were no significant differences across the groups. However, from the plot we generated above, it seems that only the Hand View group is at the 0-degree mark for changes in predicted sensory consequences. Thus, we ran t-tests that compared each group's magnitude of changes in predicted sensory consequences against 0.
...
To visualize the data, we plot the relationship of passive localization and aftereffects, as well as predictions and aftereffects, individually. Note that we did perform a Pearson product moment correlation test for these two and found a significant relationship for both, but we only include it as analyses extra to the manuscript (i.e., the main test is the multiple regression below). For the extra analyses, refer to the other R Markdown document, AnalysesSupp.

```{r fig.width=12, fig.height=4.5}
plotPropPredRelationships()
```

From the left and centre panels of the plot, we see that there is a relationship between predictions and aftereffects, as well as passive localization and aftereffects. For these plots and the regression below, we pooled data from all individuals across all groups together. One criticism could be that we are only detecting a relationship because of a group effect that is leading to a spurious correlation. However, even after taking into account the group effect, these predictors are still statistically significant. (See AnalysesSupp for these tests).
...
We found that both changes in proprioception (Beta = -0.69, p < .001) and predictions (Beta = -0.43, p < .001) were significant predictors of reach aftereffects. As a measure of effect size, we can use the squared semi-partial correlation coefficient (sr-squared). This means that 17% of the variance in reach aftereffects is accounted for by updates in predictions, after partialling out proprioceptive changes. On the other hand, 20% of the variance in reach aftereffects is accounted for by proprioceptive changes, after partialling out updates in predictions.

Importantly, we note that changes in proprioception are independent from predicted sensory consequences due to how we have measured them. Recall that proprioceptive recalibration was measured from Passive localization, while predictions were calculated as a difference of Active and Passive Localization. This process ensures predictions do not have any shared variance with proprioceptive recalibration. Furthermore, predictions and proprioceptive changes are not collinear with each other (vif = 1.09 is considered low; see AnalysesSupp for extra analyses). 
...
We note that these results do not definitively show that changes in proprioception and predictions are what cause aftereffects. That is, we cannot establish causation from this multiple regression. If we considered the correlations of each predictor with aftereffects (r-squared values from left and centre panels of plot), these values are still small in magnitude. It is possible that the significant relationships we found were due to an alternative variable or a common source (e.g. factors such as participant motivation). Therefore, although our results suggest that both proprioception and predictions independently contribute to aftereffects, these should be interpreted with caution.
