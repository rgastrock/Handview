...
Next, we want to test whether each group shows any recalibration in proprioception. We run t-tests that compare each group's magnitude of shifts in passive localization estimates against 0.
...
First, we test for the presence of a correlation, then show the data and results from a simple regression model.

## Correlation test
Here, we test whether the shifts in passive localization are correlated with the angular reach deviations of the hand in *No Cursor - Without Strategy* trials. First, we generate a rough plot to see whether there seems to be a relationship between the two variables. Then, run the test, which includes all particiants regardless of group.

```{r}
getRAEPropCorrelation()
```

We find a negative relationship between afferent-based changes in localization and reach aftereffects when excluding a strategy.

## Show the Data and Regression

Then, we show the data more formally. We plot the relationship of reach aftereffects in exclusive trials with the shifts in passive localization. Our plot will show data points from all participants, separated by group. However, we also plot a regression line with 95% confidence intervals for all participants, regardless of group.

```{r}
plotGroupCorrelations()
```

We see from the plot the negative relationship confirmed from the correlation test. Moreover, the simple regression model shows that reach aftereffects in exclusive No Cursor trials are a significant predictor (p = 0.0008, r-squared = 0.12) of shifts in passive localization.
