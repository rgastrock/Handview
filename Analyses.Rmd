...
First, we want to test whether localization did shift after training with the rotated cursor. We run a 2X2X4 mixed ANOVA with session (*rotated_b* in data; aligned and rotated) and movement type (*passive_b* in data; active and passive) as within-subjects factors and group (instructed, non-instructed, cursor jump, and hand view) as a between-subject factor.

```{r}
localizationANOVA(test='omnibus')
```

To answer the question, we see that rotated_b had a significant main effect, which suggests that localization after rotated training differed from localization before training. We also see other effects which we will investigate further.
...
We see that there is no significant difference between the groups. This suggests that passive localization changes are not affected by level of explicit learning.

NOTE: In the instance that we might be interested in testing for the effect of group on active localization changes, the function can also run the one way ANOVA on active localization changes. I do not show it here, but in that test, there is a significant difference between groups.
