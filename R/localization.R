...
plotLocalizations <- function(target='inline'){
  ...
  if (target == 'svg') {
    svglite(file='doc/fig/Fig6_localization.svg', width=8.5, height=7, pointsize=14, system_fonts=list(sans='Arial'))
  }
  
  #par(mfrow=c(1,2), mar=c(4,4,2,0.1))
  par(mar=c(4,4,1,0)) #4,4,4 and .1
...
plotPropPredRelationships <- function(target='inline') {
  ...
  #mtext('B', side=3, outer=TRUE, line=-1, adj=0, padj=1)
